...
public function hasQueries(array $query_map) : bool {
	// \rkphplib\lib\log_debug("ADatabase.hasQueries:583> query_map: ".print_r($query_map, true));
	if (!is_array($query_map)) {
		return false;
	}
...
public function buildQuery(string $table, string $type, array $kv = []) : string {
	...
	// \rkphplib\lib\log_debug("ADatabase.buildQuery:1585> table=$table, type=$type, kv: ".print_r($kv, true)."p: ".join('|', array_keys($p)));
	...
	foreach ($p as $col => $cinfo) {
		...
		// \rkphplib\lib\log_debug("ADatabase.buildQuery:1610> col=$col, val=$val");

		if ($val !== false) {
			array_push($key_list, self::escape_name($col));
			array_push($val_list, $val);
			// \rkphplib\lib\log_debug("ADatabase.buildQuery:1615> table=$table, type=$type, col=$col, val=$val");
		}
	}

	if (count($key_list) == 0) {
		// \rkphplib\lib\log_debug("ADatabase.buildQuery:1620> empty key_list - return");
		return '';
	}
	...
	// \rkphplib\lib\log_debug("ADatabase.buildQuery:1646> table=$table, type=$type, res=$res");
	return $res;
}
