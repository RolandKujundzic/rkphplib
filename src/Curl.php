...
public static function check(string $url, int $timeout = 3, string $ok_rx = '[23]0[0-8]|40[134]') : bool {
	...
	// \rkphplib\lib\log_debug("Curl::check:174> call $url");
	curl_exec($ch);
	...
	// \rkphplib\lib\log_debug("Curl::check:185> $code");
...
private function cacheOk(string $name, string $file) : bool {
	...
	// \rkphplib\lib\log_debug("Curl.cacheOk:262> $file: ".intval($res));
	return $res;
}
...
private function call_curl(string $url) : string {
	...
	// \rkphplib\lib\log_debug('Curl.call_curl:296> '.print_r($this->opt, true));
	$res = curl_exec($ch);
