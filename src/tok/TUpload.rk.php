...
public function tok_upload_init($name, $p) {
	...
	if (!empty($this->conf['scan'])) {
		$this->tok_upload_conf($name, []);
		$this->tok_upload_scan($this->conf['upload']);
	}
}
...
public function tok_upload_conf($name, $p) {
	...
	if (!isset($this->options['_default'])) {
		$this->options['_default'] = $this->conf;
	}
  ...
  if (!empty($p['upload'])) {
    ...
    $p['upload'] = $name;
		...
		throw new Exception('empty name', print_r($p, true));
	}

	if (count($p) > 0) {
		$this->tok_upload_init($name, $p);
		$this->options[$name] = $this->conf;
		// \rkphplib\lib\log_debug("TUpload.tok_upload_conf($name, ...)> this.options: ".print_r($this->options,true));
	}
...
public function tok_upload_scan($name = 'upload') {
	...
	else if (!empty($this->conf['stream'])) {
