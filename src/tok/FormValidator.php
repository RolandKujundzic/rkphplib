...
public function tok_fv2_preset(string $param, ?string $arg) : string {
	// \rkphplib\lib\log_debug("FormValidator.tok_fv2_preset:218> param=[$param] arg=[$arg]");
...
public function tok_fv2_init(string $do, array $p) : void {
	...
	if ($do == 'add') {
		...
		// \rkphplib\lib\log_debug([ "FormValidator.tok_fv2_init:319> reset, do=$do p: <1>", $p ]);
		$this->conf['current'] = [];
		$this->error = [];
		$this->example = [];
	}
	...
	// \rkphplib\lib\log_debug([ "FormValidator.tok_fv2_init:346> ($do, …) use_conf: <1>\ncurrent: <2>", $use_conf, $this->conf['current'] ]);
}
...
public function tok_fv2_check(string $ajax = '') : string {
	...
	// \rkphplib\lib\log_debug([ "FormValidator.tok_fv2_check:361> submit=$submit _REQUEST: <1>", $_REQUEST ]);
	foreach ($this->conf['current'] as $key => $value) {
	...
	if (empty($this->conf['current']['col_val'])) {
		...
		if (!empty($this->conf['current']['allow_column'])) {
			...
			// \rkphplib\lib\log_debug([ "FormValidator.tok_fv2_check:411> column=$column allow_col: <1>", $allow_col ]);
		...
		if ($path[0] == 'check') {
			...
			if (!$is_ok && !empty($req_value) && $req_value != trim($_REQUEST[$name])) {
				if (ValueCheck::run($name, trim($req_value), $check)) {
					// \rkphplib\lib\log_debug("FormValidator.tok_fv2_check:429> auto-trim $name value [$req_value]");
			...
			if (!$is_ok) {
				...
				// \rkphplib\lib\log_debug([ "FormValidator.tok_fv2_check:442> path=$key name=$name error: <1>", $this->error[$name] ]);
			}
		}
	}
	...
	// \rkphplib\lib\log_debug("FormValidator.tok_fv2_check:462> res=$res");
	return $res;
}
...
private function ajaxOutput(string $ajax) : string {
	try {
		...
		// \rkphplib\lib\log_debug("FormValidator.ajaxOutput:473> tpl:$ajax=$output");
...
private function get2NData(string $name, string $name_def) : string {
	...
	// \rkphplib\lib\log_debug([ "FormValidator.get2NData:544> <1>", $r ]);
	...
	// \rkphplib\lib\log_debug([ "FormValidator.get2NData:575> name=$name value=[$value] res=[$res] r: <1>", $r ]);
	return $res;
}
...
private function multiCheckbox(string $name, array $p) : string {
	// \rkphplib\lib\log_debug([ "FormValidator.multiCheckbox:584> name=$name p: <1>", $p ]);
	...
	// \rkphplib\lib\log_debug("FormValidator.multiCheckbox:593> name=$name entry=[$entry] entries=[$entries]");
	for ($n = 0; $n < $p['n_max']; $n++) {
		$var = $name.$n;
		...
		// \rkphplib\lib\log_debug([ "FormValidator.multiCheckbox:608> var=$var input_name=$input_name r: <1>\n$html", $r);
...
private function _fv_in_html(string $name, array $r, string $output_in = '') : string {
	...
	// \rkphplib\lib\log_debug([ "FormValidator._fv_in_html:663> name=$name res=[$res] r: <1>", $r ]);
	return $res;
}
...
public function tok_fv2_in(string $name, array $p) : string {
	...
	// \rkphplib\lib\log_debug([ "FormValidator.tok_fv2_in:680> name=$name key=$skey is_action=$is_action p: <1>", $p ]);
	...
	// \rkphplib\lib\log_debug([ "FormValidator.tok_fv2_in:716> name=$name p: <1>", $p ]);
...
private function getConf(string $key, $engine = '', bool $required = true) : string {
	...
	// \rkphplib\lib\log_debug("FormValidator.getConf:764> ($key, $engine, $required) ckey = $ckey");
	if (!isset($conf[$ckey])) {
		$res = '';
...
protected function parseInName(string $name, string $value, array &$p) : void {
	$r = conf2kv($value, '=', ',');
	// \rkphplib\lib\log_debug([ "FormValidator.parseInName:811> name=$name, value=$value, r: <1>\np: <2>", $r, $p ]);
	...
	if (!empty($r['multi'])) {
		$p = array_merge($p, $r);
		// \rkphplib\lib\log_debug([ "FormValidator.parseInName:824> name=$name, value=$value, multi p: <1>", $p ]);
		return;
	}
	...
	// \rkphplib\lib\log_debug([ "FormValidator.parseInName:916> name=$name, value=$value, p: <1>", $p ]);
}
...
protected function getInput(string $name, array $ri) : string {
	...
	// \rkphplib\lib\log_debug([ "FormValidator.getInput:946> name=$name tpl_in=$tpl_in ri: <1>", $ri ]);
	if (empty($ri['type'])) {
	...
	if (!empty($ri['tpl_in'])) {
		$input = $ri['tpl_in'];
		// \rkphplib\lib\log_debug("FormValidator.getInput:954> $input");
		unset($ri['tpl_in']);
	}
	...
	// \rkphplib\lib\log_debug([ "FormValidator.getInput:1035> name=$name, input=[$input] ri: <1>", $ri ]);
	return $input;
}
...
private function getCheckOptions(array &$p, string $name, string $str_options) : array {
	...
	// \rkphplib\lib\log_debug([ "FormValidator.getCheckOptions:1051> name=[$name] str_options=[$str_options] tpl=[$tpl] p: <1>", $p ]);
	...
	// \rkphplib\lib\log_debug("FormValidator.getCheckOptions:1069> return $html");
	return $html;
}
...
private function getOptions(array &$p, string $opt_value, string $str_options) : string {
	...
	// \rkphplib\lib\log_debug([ "FormValidator.getOptions:1082> opt_value=[$opt_value] str_options=[$str_options] p: <1>", $p ]);
	...
	// \rkphplib\lib\log_debug("FormValidator.getOptions:1130> return $html");
	return $html;
}
