...
public function tok_menu_add($level, $node) {
	...
	// \rkphplib\lib\log_debug("AMenu.tok_menu_add:124> label=$label level=$level ignore_level=".$this->ignore_level);
	if ($this->ignore_level > 0 && $level >= $this->ignore_level) {
		// \rkphplib\lib\log_debug("AMenu.tok_menu_add:126> call skipNode and return - label=$label level=$level ignore_level=".$this->ignore_level);
		$this->skipNode($node);
		return;
	}
	...
	// \rkphplib\lib\log_debug("AMenu.tok_menu_add:138> nc=$nc id=".($nc + 1)." parent=0 prev=node.".($nc - 1));
	if ($prev) {
	...
	if (isset($node['if']) && empty($node['if'])) {
		...
		// \rkphplib\lib\log_debug("AMenu.tok_menu_add:164> skipNode and return - if = false");
		$this->skipNode($node);
		return;
	}
	...
	if (!empty($node['if_table']) && !$this->hasTables($node['if_table'])) {
		$this->ignore_level = $level + 1;
		// \rkphplib\lib\log_debug("AMenu.tok_menu_add:171> skipNode and return - no such table ".$node['if_table']);
		$this->skipNode($node);
		return;
	}
	...
	// \rkphplib\lib\log_debug("AMenu.tok_menu_add:195> add node: ".print_r($node, true));
	...
	for ($i = 0; $i < count($path); $i++) {	
		...
		for ($j = 0; !$found && $j < count($this->node); $j++) {
			...
			if ($node['dir'] == $curr_path) {
				// \rkphplib\lib\log_debug("AMenu.addNodeHi:247> ($i, $j): curr_path=$curr_path node.dir=".$node['dir']);
				$this->node[$j]['hi'] = 1;
				$found = true;
