...
public function tok_upload_exists($p) {
	...
	if (empty($this->conf['save_in'])) {
		if (!empty($p['save_in'])) {
			...
			// \rkphplib\lib\log_debug("TUpload.tok_upload_exists:160> return [] - no pictures");
			return '[]';
		}
	}

	if (!Dir::exists($this->conf['save_in'])) {
		// \rkphplib\lib\log_debug("TUpload.tok_upload_exists:166> return [] - no such directory ".$this->conf['save_in']);
		return '[]';
	}
	...
	// \rkphplib\lib\log_debug("TUpload.tok_upload_exists:179> save_in=".$this->conf['save_in']." p: ".print_r($p, true));
	if (empty($p['mode'])) {
		...
		foreach ($entries as $entry) {
			...
			if (!empty($ii['width'])) {
				...
				// \rkphplib\lib\log_debug("TUpload.tok_upload_exists:194> entry=$entry tbn=$tbn_url");
...
public function tok_upload_init($name, $p) {
	...
	// \rkphplib\lib\log_debug("TUpload.tok_upload_init:270> name=[$name], p: ".print_r($p, true));
...
public function tok_upload_conf($name, $p) {
	...
	if (count($p) > 0) {
		// \rkphplib\lib\log_debug("TUpload.tok_upload_conf:331> name=[$name] p: ".print_r($p,true));
...
public function tok_upload_scan($name = 'upload') {
	...
	if ((!empty($_REQUEST['ajax']) && $_REQUEST['ajax'] != $name && $_REQUEST['ajax'] != 'upload') || isset($this->options['_done_'.$name])) {
		// \rkphplib\lib\log_debug("TUpload.tok_upload_scan:373> return - name=[$name] _REQUEST=".print_r($_REQUEST, true));
		return;
	}

	$this->conf = isset($this->options[$name]) ? array_merge($this->options['_default'], $this->options[$name]) : $this->options['_default'];
	// \rkphplib\lib\log_debug("TUpload.tok_upload_scan:378> name=[$name] this.conf: ".print_r($this->conf, true));
...
*/
	...
	// \rkphplib\lib\log_debug("TUpload.tok_upload_scan:453> return");
}
...
private function scanFiles($name) {
	...
	// \rkphplib\lib\log_debug("TUpload.scanFiles:466> name=[$name] _FILES: ".print_r($_FILES, true));
	...
	if (!isset($_FILES[$fup]) || (empty($_FILES[$fup]['name']) && empty($_FILES[$fup]['tmp_name']))) {
		// \rkphplib\lib\log_debug("TUpload.scanFiles:473> return - no single _FILES[$fup] upload");
		return '';
	} 

	if (is_array($_FILES[$fup]['name']) && count($_FILES[$fup]['name']) == 1 && 
			(empty($_FILES[$fup]['name'][0]) && empty($_FILES[$fup]['tmp_name'][0]))) {
		// \rkphplib\lib\log_debug("TUpload.scanFiles:479> exit - no multi _FILES[$fup] upload");
		return '';
	}
	...
	// \rkphplib\lib\log_debug("TUpload.scanFiles:493> _FILES[$fup]: ".print_r($_FILES[$fup], true));
...
private function removeImage() {
	...
	// \rkphplib\lib\log_debug("TUpload.removeImage:531> update_query (remove: ".$path_prefix.$remove_img[0]."): $update_query");
...
private function removeFSImages() {
	...
	for ($i = 0; $i < count($remove); $i++) {
		$file = $remove[$i];
		if (File::exists($file)) {
			// \rkphplib\lib\log_debug("TUpload.removeFSImages:564> remove $file");
			$this->options['@plugin_action'] = 1;
...
private function replaceImage() {
  ...
  $target = isset($_REQUEST['upload_'.$name.'_file']) ? $_REQUEST['upload_'.$name.'_file'] : '';
	...
	$path_prefix = empty($this->conf['save_dir']) ? '' : $this->conf['save_dir'].'/';
	// \rkphplib\lib\log_debug("TUpload.replaceImage:618> move $path_prefix$target to $path_prefix$old_img - r.images=".$r['images']);
	File::move($path_prefix.$target, $path_prefix.$old_img);

	$update_query = $db->getQuery('update_images', $r);
	// \rkphplib\lib\log_debug("TUpload.replaceImage:622> exit - update_query: $update_query");
}
...
private function saveMultipleFileUpload($fup, $max) {
	// \rkphplib\lib\log_debug("TUpload.saveMultipleFileUpload:655> fup=$fup max=$max conf: ".print_r($this->conf, true));
...
private function saveFileUpload($fup) {
	// \rkphplib\lib\log_debug("TUpload.saveFileUpload:705> fup=$fup conf: ".print_r($this->conf, true));
	Dir::create($this->conf['save_in'], 0777, true);
...
private function getSaveAs($upload_file, $temp_file, $nc = 0) {
	...
	// \rkphplib\lib\log_debug("TUpload.getSaveAs:819> upload_file=$upload_file temp_file=$temp_file nc=$nc base=$base suffix=$suffix fsize=$fsize");
...
*/
	...
	// \rkphplib\lib\log_debug("TUpload.getSaveAs:892> return [$res]");
	return $res;
}
