...
public function tok_tpl(array $p, ?string $arg) : string {
	...
	// \rkphplib\lib\log_debug("TBase.tok_tpl:454> return $tpl"); 
	return $tpl;
}
...
public function setVar(string $name, ?string $value, int $flag = 0) : void {
	// \rkphplib\lib\log_debug([ "TBase.setVar:510> flag=$flag, name=$name value: <1>", $value ]);
...
public function getVar(string $name, string $name2 = '') {
	...
	// \rkphplib\lib\log_debug([ "TBase.getVar:538> $name: <1>", $res ]);
	return $res;
}
...
public function setVarHash(string $name, array $p) : void {
	if (substr($name, -1) != '!') {
		// \rkphplib\lib\log_debug([ "TBase.setVarHash:549> name=$name p: <1>", $p ]);
	...
	// \rkphplib\lib\log_debug([ "TBase.setVarHash:565> name=$name p: <1>\nold: <2>", $p, $old_p ]);
...
private function tableRow(array $cols, array $p) : string {
	...
	// \rkphplib\lib\log_debug("TBase.tableRow:668> return $res");
	return $res;
}
...
public function tok_keep(string $txt) : string {
	// \rkphplib\lib\log_debug("TBase.tok_keep:860> return $txt");
	return $txt;
}
...
public function tok_link(array $name_list, array $p) : string {
	// \rkphplib\lib\log_debug("TBase.tok_link:1034> name_list: ".print_r($name_list, true)."\np: ".print_r($p, true));
	...
	// \rkphplib\lib\log_debug("TBase.tok_link:1137> return $res");
	return $res;
}
...
public static function decodeHash(string $data, bool $export_into_req = false) {
	...
	// \rkphplib\lib\log_debug("TBase::decodeHash:1227> return ".print_r($res, true));
	return $res;
}
...
public function tok_esc(string $param, ?string $arg) : ?string {
	...
	// \rkphplib\lib\log_debug("TBase.tok_esc:1594> return [$arg]");
	return $arg;
}
...
public function tok_tf(array $p, string $arg) : void {
	...
	// \rkphplib\lib\log_debug([ "TBase.tok_tf:2069> ta=<1> p: <2>", $ta, $p ]);
	if (count($p) == 1) {
		if ($p[0] === '') {
			$tf = !empty($ta);
		}
	...
	// \rkphplib\lib\log_debug([ "TBase.tok_tf:2112> do=<1> tf=<2> ap: <3>", $do, $tf, $ap ]);
	if (empty($do)) {
	...
	if ($do == 'cmp') {
		...
		// \rkphplib\lib\log_debug([ "TBase.tok_tf:2191> tf=<1> set: <2>", $tf, $set ]);
	}
	else if ($do == 'and' || $do == 'or') {
		$apn = count($ap);
...
public function tok_true(string $val, string $out) : string {
	...
	// \rkphplib\lib\log_debug('TBase.tok_true:2261> val='.print_r($val, true).' tf='.print_r($tf, true));
...
public function tok_log(string $txt) : void {
	\rkphplib\lib\log_debug("TBase.tok_log:2289> $txt"); // @keep
}
