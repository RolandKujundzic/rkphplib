...
public function tok_link(array $name_list, array $p) : string {
	// \rkphplib\lib\log_debug("TBase.tok_link:1010> name_list: ".print_r($name_list, true)."\np: ".print_r($p, true));
	...
	// \rkphplib\lib\log_debug("TBase.tok_link:1109> return $res");
	return $res;
}
...
public static function decodeHash(string $data, bool $export_into_req = false) {
	...
	// \rkphplib\lib\log_debug("TBase::decodeHash:1199> return ".print_r($res, true));
	return $res;
}
...
private function applyFilter(string $tag, string $value) : string {
	// \rkphplib\lib\log_debug("TBase.applyFilter:1493> tag=$tag value=[$value]");
	...
	foreach ($this->_conf['filter.'.$tag] as $filter) {
		...
		// \rkphplib\lib\log_debug("TBase.applyFilter:1520> filter=$filter value=[$value]");
	}
...
public function tok_esc(string $param, ?string $arg) : ?string {
	...
	if (is_null($arg) || $arg === 'null' || $arg === 'NULL') {
		// \rkphplib\lib\log_debug("TBase.tok_esc:1556> return NULL");
		return 'NULL';
	}
	...
	// \rkphplib\lib\log_debug("TBase.tok_esc:1566> return [$arg]");
	return $arg;
}
...
public function tok_plugin(string $ns, array $p) : void {
	foreach ($p as $plugin) {
		...
		// \rkphplib\lib\log_debug("TBase.tok_plugin:1915> require_once '$cpath'; new $obj();");
		require_once $cpath;
...
public function tok_tf(array $p, string $arg) : void {
	...
	// \rkphplib\lib\log_debug([ "TBase.tok_tf:2041> ta=<1> p: <2>", $ta, $p ]);
	if (count($p) == 1) {
		if ($p[0] === '') {
			$tf = !empty($ta);
		}
...
public function tok_true(string $val, string $out) : string {
	...
	// \rkphplib\lib\log_debug('TBase.tok_true:2233> val='.print_r($val, true).' tf='.print_r($tf, true));
...
public function tok_log(string $txt) : void {
	\rkphplib\lib\log_debug("TBase.tok_log:2261> $txt"); // @keep
}
